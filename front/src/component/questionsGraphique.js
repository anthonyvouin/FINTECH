...
import dashboard_test from '../media/dashboard_test.png';

const QuestionsGraphique = (props) => {
  ...
  const changeImage = () => {
    try {
        document.querySelector(".question_container_question").style.dislay = "block";
        document.querySelector(".question_container_image").remove();
    } catch (error) {
        alert("Petit problème durant l'envoi de vos réponses");
    }
  };
 
  return (
    <>
    <div id={"question_container_" + props.question.id} className="question_container_image grid max-w-screen-xl px-4 py-8 mx-auto lg:gap-8 gap-2 xl:gap-0 lg:py-16">
      <div className="text-center">
        <h1 className="max-w-2xl mb-4 text-xl font-extrabold leading-none ">{props.question.title}</h1>
        <p className="mt-3 text-lg leading-relaxed text-slate-400">Cliquez sur l'image quand vous avez trouver le mois durant lequel vous avez été le plus actif</p>
      </div>

      <div className="mr-auto w-full place-self-center" onClick={changeImage}>
        <img src={dashboard_test} alt="" className="portfolio__img" />
      </div>
    </div>
    

    <div id={"question_container_" + props.question.id} 
    className={`question_container_question grid max-w-screen-xl px-4 py-8 mx-auto lg:gap-8 gap-2 xl:gap-0 lg:py-16 lg:grid-cols-12 ${props.question.id % 2 !== 0 ? "lg:flex-row-reverse lg:flex-wrap" : ""}`}>
...
export default QuestionsGraphique;